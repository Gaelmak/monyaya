...
export const TrainingsView = ({ data, userId, sessionName, myLearnings, className }: Props) => {
  ...
  return (
    <Container className={clsx(className)}>
      {visibleData.map(({ id, createdAt, image, name, price, _count, user, courses }) => (
        <Container key={id} className="group p-4 hover:cursor-pointer rounded-2xl overflow-hidden flex flex-col gap-2 justify-between bg-white animate">
          <Link href={`/trainings/training/${id}`} className="rounded-2xl w-full relative md:w-auto aspect-video overflow-hidden flex justify-center items-center">
            <Image width={100} height={100} src={image || RekreationPaysage} alt="Training image" className="h-full w-full object-cover group-hover:scale-150 animate" />
          </Link>
          ...
          <Container className="flex justify-between">
            ...
            <Container className="flex items-center gap-2">
              <SearchResultButtons
                ...
                amLearner={myLearnings?.some(obj => obj.trainingId === id) || false}
                status={myLearnings?.find(obj => obj.trainingId === id)?.status || null}
              />
