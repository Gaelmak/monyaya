  ...
  async function addLearners(trainingId: string, userId: string) {
    ...
    if (add.status === 200) {
      toast({
        ...
        description: (
          <Typography component="p" variant="body-sm">
            La formation a été ajoutée avec succès et est en attente de
            validation de la part du yaya. Vous récévrez une notification de la
            part du yaya concernant la suite.
          </Typography>
        ),
      });
      stopLoading();
      ...
      toast({
        ...
        description: (
          <Typography component="p" variant="body-sm">
            Une erreur est survenue durant l'ajout de la formation. Veuillez
    ...
    <Container className="flex flex-row gap-2">
      {amLearner ? (
        ...
        <Buttons
          ...
          isLoading={isLoading}
          buttonType="action"
          action={() => addLearners(trainingId!, userId!)}
        />
      ) : (
        <Buttons
          Icon={Plus}
...
export const SearchResultTrainer = ({
  image = DefaultAvatar,
  name,
  fullName,
  isMyAccount = false,
