...
import { Archive, ArchiveRestore, CalendarCheck, CalendarClock, CalendarX, Check, Plus, Trash2 } from "lucide-react";
...
export const ValidationButton = ({status, trainingId, amLearner = false}: Props) => {
  ...
  async function handleTraining (id: string, status: string) {
    ...
    if(add.status === 200) {
      toast({
        ...
        description: <Typography component="p" variant="body-sm">L'état de la formation a été modifié avec succès.</Typography>,
      })
      stopLoading()
    } else {
      toast({
        ...
        <Typography component="p" variant="body-sm">
          Une erreur est survenue durant la modification de l'état de la formation. Veuillez recommencer l'opération.
        </Typography>,
      })
      stopLoading()
    }
  ...
  async function rejectTraining (id: string) {
    ...
    if(add.status === 200) {
      toast({
        variant: "success",
        title: "Formation ajoutée !",
        description: <Typography component="p" variant="body-sm">La formation a été annulée avec succès.</Typography>,
      })
      stopLoading()
    } else {
      toast({
        ...
        description: 
        <Typography component="p" variant="body-sm">
          Une erreur est survenue durant l'annulation de la formation. Veuillez recommencer l'opération.
        </Typography>,
      })
      stopLoading()
    }
  ...
  return(
    <Container className="flex flex-row gap-2">
        ...
        <Container className="flex flex-row gap-2">
          {
            ...
            : status === "ARCHIVED" ?
            <Buttons Icon={Archive} className="text-white bg-red-400 hover:bg-red-400" isLoading={isLoading}/>
            :
      ...
      {
        status === 'PENDING' ?
          <Buttons Icon={Check} isLoading={isLoading} buttonType="action" action={() => handleTraining(trainingId!, "APPROVED")}/>
        :
        status !== "ARCHIVED" &&
          <Buttons Icon={ArchiveRestore} isLoading={isLoading} buttonType="action" action={() => handleTraining(trainingId!, "ARCHIVED")}/>
